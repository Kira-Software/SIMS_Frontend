...
const Registrarspecial = () => {
  //  const user = useSelector(state => state.authreducer.user)
  const dispatch = useDispatch();
  const specialentrylist = useSelector((state) => state.registrar.specialentrylist);
  ...
  const handlesubmit = () => {
    ...
    dispatch(
      createspecialentry(Id, Department, Year, Semister, Section, Coursename)
    );
  };
        ...
        <Grid item xs={12} lg={6} xl={6}>
          ...
          <div style={{ display: displaylist, marginTop: 50 }}>
            <div className="card" style={{ marginTop: 50, marginBottom: 100 }}>
              ...
              {specialentrylist.length !== 0
                ? specialentrylist.map((item, itemidx) => {
...
export default Registrarspecial;
